...
var _chip_t: float = 0.0   # Chip malfunction efekti sayacı
...
func _draw() -> void:
	if is_dead or is_in_group("allies"):
		return
	# Chip arıza göstergesi — hafif EM titreşim halkası
	...
	var radius:  float = 6.0 + pulse * 2.0
	draw_arc(Vector2(0.0, -16.0), radius, 0.0, TAU, 10,
			 Color(0.0, 0.85, 1.0, alpha), 1.2)
	if flicker > 0.87:
		var sy: float = -18.0 + sin(_chip_t * 7.0) * 8.0
		draw_line(Vector2(-5.0, sy), Vector2(5.0, sy),
